...
if($pluginName!='JError'){/* !!! - continue from here */
	...
	/**
	 * Generates a html-header for standard-includes.
	 */
	private function frameworkIncludes()
	{
		...
		//die('header '.var_dump($HTML_HEAD_FILES_EXCLUSIONS));
		// http static files
	...
	public function runModule($moduleName)
	{
		...
		if (Config::getConfig("module_dir") != "") $moduleDir = Config::getConfig("module_dir");
	...
	{
		$k = md5($key);
		if ($this->posts[$k]) return $this->posts[$k];
		$p = new PostInput($key, "");
		return $p;
	}
	/**
	 ...
	 * @return Array of all posts variables.
	 */
	 ...
	 * @param $path The path where it is valid.
	 * @param $domain The domain where the cookie is valid.
	 * @param $time The expiration time.
	 */
	public function createCookie($name = "", $value = "", $path = "", $domain = "", $time = 0)
	{
